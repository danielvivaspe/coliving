...
def get_prediction(userID, model, activityID):
    '''
    ...
    '''
    return model.predict(userID, activityID)

def get_predictions(userId, dataframe, model, n_recommendations, column_iid= None, column_uid= None):
    """
    ...
    for item in items_no_finished:
        preds.append(model.predict(uid=userId, iid=item))
